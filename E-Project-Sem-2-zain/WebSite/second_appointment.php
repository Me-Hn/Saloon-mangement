    ...
    <section class="mainsec">
      <section class="persec">
        ...
        <form method="post">
          ...
          <input type="hidden" value="<?php echo $_SESSION['Cid'] ?>" class="form-control inputfeild" name="cid"
          ...
          <div class="mb-3">
            ...
            <input type="date" class="form-control inputfeild" name="cdate" id="exampleInputPassword1" required>
          </div>
          <div class="mb-3">
        ...
        <section class="sersec">
          ...
          <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label ">Hair Style :</label>
            <select name="hser" class="form-select inputfeild" aria-label="Default select example" required>

              <option value="1">Select Hair Style</option>
              <?php
              $gethairstyle = mysqli_query($con, "SELECT * FROM `hair_style` ");
              while ($hairrow = mysqli_fetch_array($gethairstyle)) {
                ?>
                <option value="<?php echo $hairrow[1] ?>"><?php echo $hairrow[1] ?></option><?php
              } ?>
            </select>
          </div>

          <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Beard/Khat :</label>
            <select name="bser" class="form-select inputfeild" aria-label="Default select example" required>

              <option value="1">Select Beard Style</option>
              <?php
              $getbeard = mysqli_query($con, "SELECT * FROM `beard_style`");
              while ($beardrrow = mysqli_fetch_array($getbeard)) {
                ?>
                <option value="<?php echo $beardrrow[1] ?>"><?php echo $beardrrow[1] ?></option><?php
              } ?>
            </select>
          </div>
          ...
          <!-- Face Treatment -->
          <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Face Treatment :</label>
            <select name="fser" class="form-select inputfeild" aria-label="Default select example" required>

              <option value="1">Select Face Treatment</option>
              <?php
              $getface = mysqli_query($con, "SELECT `ID`, `Name` FROM `face_treatment`");
              while ($facerow = mysqli_fetch_array($getface)) {
                ?>
                <option value="<?php echo $facerow[1]; ?>"><?php echo $facerow['Name']; ?></option>
                <?php
              }
              ?>
            </select>
          ...
          <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label">Hair Dying :</label>
            <select name="hhser" class="form-select inputfeild" aria-label="Default select example" required>

              <option value="1">Select Hair Treatment</option>
              <?php
              $gethair = mysqli_query($con, "SELECT * FROM `hair_teatment`");
              while ($dyingrow = mysqli_fetch_array($gethair)) {
                ?>
                <option value="<?php echo $dyingrow[1] ?>"><?php echo $dyingrow[1] ?></option><?php
              } ?>
            </select>
          </div>

        
        <button type="submit" name="submitbtn" class="btn btn-primary submit-btn">Confirm Your Order</button>
        ...
        if (isset($_POST['submitbtn'])) {
          ...
          $inputdata = mysqli_query($con, "INSERT INTO `appointement`(`client_id`, `Name`, `Email`, `Slote`, `Date`, `Phone_No`, `Hair_Style`, `Beard_Style`, `Face_Treatment`, `Hair_Treatment`, `Deal_No`)
            VALUES ('$cccid', '$ccname', '$ccemail', '$ccslote', '$ccdate', '$ccphone', '$chser', '$cbser', '$cfser', '$chhser','No')");
